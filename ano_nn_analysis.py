...
def loop(model, dataloader, optimizer=None, rank=None, k=0.0, use_weight=True):
    is_eval_mode_for_loop = not optimizer # Consider as evaluation purpose if no optimizer
...
def train_and_validate_model(k_value,
    ...
    if not any(ep == EPOCHS for ep in report_epochs) and EPOCHS > 0 and EPOCHS < report_epochs[0] if report_epochs else True : # When EPOCHS is smaller than minimum value in report_epochs
    ...
    if not min_losses_at_epochs and EPOCHS > 0 : # If report_epochs is empty and EPOCHS > 0 (e.g. EPOCHS=50)
    ...
    for epoch in pbar:
        ...
        pbar.set_postfix({
            ...
            "TrainMSE(clean)": f"{train_mse_clean:.4f}", # Output for reference
    ...
    for target_epoch_val in min_losses_at_epochs.keys():
        ...
        if effective_epochs > 0 : # Calculate only when there are valid epochs
            min_losses_at_epochs[target_epoch_val]['train'] = np.min(loss_list[:effective_epochs, 0]) # minimum of noisy train mse
            min_losses_at_epochs[target_epoch_val]['val'] = np.min(loss_list[:effective_epochs, 1])   # minimum of clean val mse
...
def visualize_and_print_stats(loss_list, k_value, min_losses_at_epochs, last_epoch):
    ...
    fig = plt.figure(figsize=(10, 5)) # Slightly adjust graph size
    ...
    if len(loss_list) > start_epoch_plot:
        plt.plot(epochs_to_plot, loss_list[start_epoch_plot:, 1], 'b--', linewidth=2, label='Validation MSE') # Blue dashed line
        plt.plot(epochs_to_plot, loss_list[start_epoch_plot:, 0], 'r-', linewidth=1, label='Train MSE')      # Red solid line
    else:
    ...
    for target_epoch in [100, 200, 300]:
        ...
        # Print only when value is not infinity (inf)
